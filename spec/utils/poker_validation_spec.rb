...
RSpec.describe PokerValidation, type: :module do
  include PokerValidation

  describe "#validate_cards" do
    subject { PokerValidation.validate_cards(cards) }
    ...
    context "when the suits of all cards are invalid" do
    ...
    context "when the suit of particular card is invalid" do
      let(:cards) { "C1 K3 S8 D10 C8" }
      it { is_expected.to eq ["2番目のカード指定文字が不正です。(K3)"] }
    end

    context "when the numbers of all cards are invalid" do
    ...
    context "when some cards with an invalid suit and an invalid number are entered" do
